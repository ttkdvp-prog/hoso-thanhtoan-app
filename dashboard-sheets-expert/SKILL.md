...
- **Fail Gracefully on Missing API:** Always add a clear, user-friendly error message if `CONFIG.url` or `VITE_APPS_SCRIPT_URL` is empty or holds a placeholder like `"PASTE_WEB_APP_URL_HERE"`. Do not let the app crash with cryptic JSON parsing errors.
- **Full CRUD by Default:** Always implement Create, Read, Update, and Delete on the dashboard UI and backend. If integrating with AppSheet, use Physical Deletion (`sheet.deleteRow()`) instead of Soft Deletion (`deleted: true`) to ensure AppSheet syncs correctly, unless the user explicitly requests otherwise. Keep complex nested data (like child lists) editable in AppSheet, but expose basic fields in the dashboard edit form.
- **Dynamic Drawers (No Cut-offs):** The detail/edit drawer must dynamically loop over all data fields returned by the API (e.g., using `Object.keys(row)`), not just the fields explicitly defined in the `COLUMNS` array. This ensures no data fields are "cut off" or hidden when the user opens a record to view or edit.
